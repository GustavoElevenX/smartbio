...
export async function POST(request: Request) {
  ...
  const { error } = await supabase.from("leads").insert({ workspace_id: project.workspace_id, project_id: parsed.data.projectId, session_id: session?.id || null, name: parsed.data.name || null, email: parsed.data.email || null, phone: parsed.data.phone || null, company: parsed.data.company || null, status: "new", source: parsed.data.source || null, recommendation: parsed.data.recommendation || null, answers: parsed.data.answers });
