...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rate = await consumeRateLimit("public-booking-cancel", publicRateLimitIdentifier(request, { objectId: id, sessionId: typeof candidate.sessionId === "string" ? candidate.sessionId : undefined }), rateLimitRules.publicFormSubmit, { failClosed: true });
  ...
  const parsed = bookingChangeSchema.safeParse(raw);
  ...
  if (booking) await enqueueProjectNotification(booking.project_id, "booking.cancel_requested", "booking", id, booking.visitor_data || {}).catch(() => undefined);
