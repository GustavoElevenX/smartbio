...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (uploadError) { logError("quote_attachment_upload_failed", { requestId, route: requestPathname(request), code: uploadError.message }); return respond(apiError("Não foi possível enviar a imagem.", 400, "upload_failed")); }
