...
export async function GET(
  _: Request,
  { params }: RouteContext<"/api/projects/[projectId]/activations">,
) {
...
export async function POST(
  ...
  const parsed = activationCreateSchema.safeParse(raw);
  if (!parsed.success) return validationError(parsed.error);
  try {
    return apiSuccess(
      {
        activation: await new ActivationService(database).create({
          workspaceId: actor.workspaceId,
          projectId,
    ...
    return apiError(
      error instanceof Error
        ? error.message
        : "Não foi possível criar a ativação.",
      400,
