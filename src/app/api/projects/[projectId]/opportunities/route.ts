...
export const PATCH = withAuthenticatedActor(async (request, context: { params: Promise<{ projectId: string }> }, actor) => {
  ...
  const { data: current } = await supabase.from("commercial_opportunities").select("status,first_handled_at,session_id,project_version_id,conversion_goal_id,entry_point_id,destination_id").eq("id", input.id).eq("project_id", projectId).eq("workspace_id", actor.workspaceId).single();
  ...
  if ((input.status === "converted" || input.status === "lost") && current?.status !== input.status) await supabase.from("analytics_events").insert({ project_id: projectId, project_version_id: current?.project_version_id || null, session_id: current?.session_id || null, event_name: input.status === "converted" ? "conversion_confirmed" : "conversion_lost", conversion_goal_id: current?.conversion_goal_id || null, entry_point_id: current?.entry_point_id || null, destination_id: current?.destination_id || null, metadata: input.status === "converted" ? { confirmedValue: input.confirmedValue } : { lossReason: input.lossReason } });
  if (input.status === "converted" && current?.status !== "converted") void recordProjectLifecycleMilestone(supabase, { eventName: "first_conversion_confirmed", projectId, metadata: { versionId: current?.project_version_id || "unknown" } }).catch(() => undefined);
