...
const viewSchema = z.enum(["overview", "goals", "entries", "pages", "destinations", "funnel"]);
export const GET = withAuthenticatedActor(async (request, context: { params: Promise<{ projectId: string; view: string }> }, actor) => {
  ...
  const events = eventRows.map((row) => ({ id: String(row.id), projectId, visitorId: "", sessionId: String(row.session_id || ""), eventName: row.event_name, stepId: row.step_id ? String(row.step_id) : undefined, optionId: row.option_id ? String(row.option_id) : undefined, conversionGoalId: row.conversion_goal_id ? String(row.conversion_goal_id) : undefined, entryPointId: row.entry_point_id ? String(row.entry_point_id) : undefined, destinationId: row.destination_id ? String(row.destination_id) : undefined, presencePageId: row.presence_page_id ? String(row.presence_page_id) : undefined, presenceSectionId: row.presence_section_id ? String(row.presence_section_id) : undefined, metadata: row.metadata as Record<string, unknown>, createdAt: String(row.created_at) })) as AnalyticsEvent[];
  ...
  opportunities.forEach((item, index) => { const row = (rows.opportunities as Array<Record<string, unknown>>)[index]; item.presencePageId = row.presence_page_id ? String(row.presence_page_id) : item.attribution?.presencePageId; item.presenceSectionId = row.presence_section_id ? String(row.presence_section_id) : item.attribution?.presenceSectionId; });
  const analytics = buildConversionAnalytics({ events, opportunities, goals: project?.conversionGoals, entries: project?.entryPoints, pages: project?.presence?.pages });
  if (view === "goals") return apiSuccess(analytics.byGoal); if (view === "entries") return apiSuccess(analytics.byEntry); if (view === "pages") return apiSuccess({ pages: analytics.byPage, sections: analytics.bySection }); if (view === "funnel") return apiSuccess(analytics.funnel);
