...
import { structuredJourneyQuestionSchema } from "@/features/ai-setup/ai-setup.schema";

const answerSchema = z.object({ key: z.string().trim().min(1).max(120), value: z.union([z.string().trim().min(1).max(4000), z.array(z.string().trim().min(1).max(500)).min(1).max(30), z.array(structuredJourneyQuestionSchema).min(1).max(6), z.number().finite(), z.boolean()]) });
