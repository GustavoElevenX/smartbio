export class VirouAIError extends Error {
  constructor(message: string, readonly code: string, readonly retryable = false, options?: ErrorOptions) {
    super(message, options);
    this.name = "VirouAIError";
  }
}

export class AIConfigurationError extends VirouAIError {
...
export function normalizeAIError(error: unknown) {
  if (error instanceof VirouAIError) return error;
  ...
  return new VirouAIError(value?.message || "Falha temporária ao usar a IA.", code, retryable, { cause: error });
}
