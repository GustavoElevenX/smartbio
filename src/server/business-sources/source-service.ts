...
function detectedType(buffer: Buffer, name: string): { type: "pdf" | "image" | "csv" | "text"; mimeType: string } { if (buffer.subarray(0, 5).toString() === "%PDF-") return { type: "pdf", mimeType: "application/pdf" }; if (buffer.subarray(0, 8).equals(Buffer.from([137,80,78,71,13,10,26,10]))) return { type: "image", mimeType: "image/png" }; if (buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff) return { type: "image", mimeType: "image/jpeg" }; if (buffer.subarray(0, 4).toString() === "RIFF" && buffer.subarray(8, 12).toString() === "WEBP") return { type: "image", mimeType: "image/webp" }; if (buffer.subarray(0, Math.min(buffer.length, 4096)).includes(0)) throw new Error("Formato de arquivo não permitido."); const extension = name.toLowerCase().split(".").pop(); if (extension === "csv") return { type: "csv", mimeType: "text/csv" }; if (extension === "txt") return { type: "text", mimeType: "text/plain" }; throw new Error("Assinatura ou extensão de arquivo não permitida. Converta DOC/DOCX para PDF."); }
