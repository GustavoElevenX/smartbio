...
export interface OpportunityFactoryInput {
  ...
  activationId?: string; benefitClaimId?: string; customerIdentityId?: string;
}
...
export function createOpportunity(input: OpportunityFactoryInput): CommercialOpportunity {
  ...
  return { id: opportunityIdempotencyKey(input), workspaceId: input.workspaceId, projectId: input.projectId, projectName: input.projectName, sessionId: input.sessionId,
    ...
    entryPointId: input.entryPointId, destinationId: input.destinationId, presencePageId: input.presencePageId || input.attribution?.presencePageId, presenceSectionId: input.presenceSectionId || input.attribution?.presenceSectionId, activationId: input.activationId, benefitClaimId: input.benefitClaimId, customerIdentityId: input.customerIdentityId, attribution: input.attribution, metadata: input.metadata || {}, createdAt: now, updatedAt: now };
}
