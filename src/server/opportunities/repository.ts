...
export async function upsertOpportunity(supabase: SupabaseClient, opportunity: CommercialOpportunity) {
  const { data, error } = await supabase.from("commercial_opportunities").upsert({ workspace_id: opportunity.workspaceId, project_id: opportunity.projectId,
    session_id: opportunity.sessionId || null, conversion_goal_id: opportunity.conversionGoalId || null, entry_point_id: opportunity.entryPointId || null, presence_page_id: opportunity.presencePageId || null, presence_section_id: opportunity.presenceSectionId || null,
