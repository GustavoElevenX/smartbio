...
export async function createCommercialNotification(input: CommercialNotificationInput) {
  if (!isCommercialNotificationEvent(input.eventKey)) {
    throw new Error("Evento de notificação desconhecido.");
  }
  const { data, error } = await database().from("notification_outbox").upsert({
...
export async function enqueueProjectNotification(
  ...
  try {
    ...
    const { data: project, error } = await client
      ...
      .eq("id", projectId)
      .maybeSingle();
    if (error || !project) throw new Error("Projeto da notificação não encontrado.");
    ...
    console.error("notification_outbox_enqueue_failed", { projectId, eventKey, objectType, objectId, message: sanitizedError(error) });
...
export async function deliverNotification(input: CommercialNotificationInput) {
  ...
  for (const userId of recipients) {
    ...
    if (preference.email) {
      try {
        const { data: previousDelivery } = await database().from("notification_deliveries").select("status").eq("notification_id", notification.id).eq("channel", "email").maybeSingle();
        if (previousDelivery?.status === "sent") { results.push(notification); continue; }
...
export function notificationRetryDelaySeconds(attempt: number) {
  return [0, 60, 5 * 60, 30 * 60, 2 * 60 * 60][Math.max(0, attempt - 1)] || 2 * 60 * 60;
}
...
export async function processNotificationOutboxBatch(
  ...
  const summary = { claimed: claimed.length, completed: 0, retried: 0, dead: 0 };
  ...
  for (const row of claimed) {
    try {
      ...
      const { error: completedError } = await client.from("notification_outbox").update({
        status: "completed",
        locked_at: null,
        locked_by: null,
        last_error: null,
      }).eq("id", row.id).eq("locked_by", workerId);
      if (completedError) throw new Error("Não foi possível concluir o item da outbox.");
      summary.completed += 1;
    } catch (error) {
      const dead = row.attempts >= 5;
      const availableAt = new Date(Date.now() + notificationRetryDelaySeconds(row.attempts + 1) * 1000).toISOString();
