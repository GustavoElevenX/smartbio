...
export async function loadConversionAnalyticsRows(supabase: SupabaseClient, projectId: string, filters: AnalyticsFilters = {}) {
  let eventQuery = supabase.from("analytics_events").select("id,project_id,project_version_id,event_name,session_id,step_id,option_id,conversion_goal_id,entry_point_id,destination_id,presence_page_id,presence_section_id,metadata,created_at").eq("project_id", projectId);
  let opportunityQuery = supabase.from("commercial_opportunities").select("id,project_id,project_version_id,session_id,conversion_goal_id,entry_point_id,destination_id,presence_page_id,presence_section_id,source_type,source_id,status,title,estimated_value,confirmed_value,currency,attribution,metadata,created_at,updated_at").eq("project_id", projectId);
  ...
  const versions = await supabase.from("project_versions").select("id,version_number,created_at").eq("project_id", projectId).order("version_number", { ascending: false });
  if (versions.error) throw new Error(versions.error.message);
  return { events: events.data || [], opportunities: opportunities.data || [], versions: versions.data || [] };
}
