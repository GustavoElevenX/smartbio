...
export async function loadConversionAnalyticsRows(supabase: SupabaseClient, projectId: string, filters: AnalyticsFilters = {}) {
  let eventQuery = supabase.from("analytics_events").select("id,project_id,event_name,session_id,step_id,option_id,conversion_goal_id,entry_point_id,destination_id,metadata,created_at").eq("project_id", projectId);
  let opportunityQuery = supabase.from("commercial_opportunities").select("id,project_id,session_id,conversion_goal_id,entry_point_id,destination_id,source_type,source_id,status,title,estimated_value,confirmed_value,currency,attribution,metadata,created_at,updated_at").eq("project_id", projectId);
