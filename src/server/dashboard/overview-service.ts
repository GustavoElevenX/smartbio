...
export async function getWorkspaceOperationalOverview(actor: AuthenticatedActor): Promise<WorkspaceOperationalOverview> {
  ...
  if (error) throw new Error(`Não foi possível carregar a visão operacional: ${error.message}`);
  if (experiments.error) throw new Error(`Não foi possível carregar os aprendizados: ${experiments.error.message}`);
  ...
  const learnings = (experiments.data || []).map((experiment) => {
