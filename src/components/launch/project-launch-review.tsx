...
export function ProjectLaunchReview({ projectId }: { projectId: string }) {
  ...
  return <div className="mx-auto max-w-[1480px] animate-enter">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[#dfe6ee] bg-white px-5 py-4 sm:px-6">
      ...
      <div className="flex flex-wrap gap-2"><Link href={`/app/projects/${project.id}/site?mode=simple`} className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#d7e1ec] bg-white px-4 text-sm font-semibold text-[#07172f] hover:border-[#9fc3ff] hover:bg-[#f7fbff]"><Pencil size={16} />Editar página</Link><Button onClick={() => setPublishOpen(true)}><Rocket size={16} />{project.status === "published" ? "Publicar alterações" : "Publicar"}</Button></div>
    </header>
    ...
    <div className="px-1 py-8 sm:px-2 lg:py-10">
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className="min-w-0 border border-[#b9c7d6] bg-white" style={{ clipPath: "polygon(0 0, calc(100% - 20px) 0, 100% 20px, 100% 100%, 0 100%)" }}>
          ...
          <div className="h-[720px] overflow-auto bg-[#dfe5eb] p-3 sm:p-5"><div className={`mx-auto min-h-full overflow-hidden bg-white shadow-[0_24px_70px_rgba(7,23,47,.16)] transition-[width] ${device === "mobile" ? "w-[390px] max-w-full" : "w-full"}`}>{home ? <PublicPresencePage project={project} page={home} preview /> : <div className="grid min-h-[520px] place-items-center p-8 text-center"><div><h2 className="text-xl font-extrabold">A página ainda não foi criada</h2><Link href={`/app/projects/${project.id}/site?mode=simple`} className="mt-4 inline-flex text-sm font-bold text-[#0054fc]">Criar página</Link></div></div>}</div></div>
        </section>
