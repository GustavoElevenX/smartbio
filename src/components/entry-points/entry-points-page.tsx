...
export function EntryPointsPage({ projectId }: { projectId: string }) {
  ...
  return <div className="animate-enter">
    <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><Link href={`/app/projects/${project.id}`} className="inline-flex items-center gap-2 text-xs font-bold text-[#74727d]"><ArrowLeft size={14} /> {project.name}</Link><div className="mt-4 flex items-center gap-3"><span className="grid size-11 place-items-center rounded-[15px] bg-[#eae6ff] text-[#5c4ed5]"><Link2 /></span><div><h1 className="text-3xl font-extrabold tracking-[-.045em]">Entradas</h1><p className="mt-1 text-sm text-[#74727d]">Escolha a superfície certa para cada canal e campanha.</p></div></div></div><div className="flex gap-2"><Button variant="secondary" onClick={addEntry}><Plus /> Entrada</Button><Button onClick={() => void save()} disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : saved ? <Check /> : <Save />}{saving ? "Salvando" : saved ? "Salvo" : "Salvar"}</Button></div></header>
    <section className="mt-7 grid gap-5 xl:grid-cols-[.82fr_1.18fr]"><div><h2 className="text-lg font-extrabold">Links publicados</h2><div className="mt-4 grid gap-3">{entries.length ? entries.map((entry) => <button type="button" key={entry.id} aria-pressed={selectedId === entry.id} className={`cursor-pointer rounded-[22px] text-left ${selectedId === entry.id ? "ring-2 ring-[#6d5ef5] ring-offset-2" : ""}`} onClick={() => setSelectedId(entry.id)}><EntryPointCard entry={entry} goal={goals.find((goal) => goal.id === entry.conversionGoalId)} url={entryPointUrl(project.slug, entry)} /></button>) : <div className="rounded-[20px] border border-dashed border-[#d9d6e1] p-10 text-center"><strong>Nenhuma entrada criada</strong><p className="mt-2 text-xs text-[#77747f]">Crie um link para bio, anúncio, story ou QR code.</p></div>}</div></div>
      <div>{selected ? <div className="grid gap-4"><div className="rounded-[24px] border border-[#e2e0e8] bg-white p-5 sm:p-6"><div className="flex items-center justify-between"><h2 className="font-extrabold">Configurar entrada</h2><Button variant="ghost" size="icon" aria-label="Excluir entrada" onClick={() => { setEntries((items) => items.filter((entry) => entry.id !== selected.id)); setSelectedId(undefined); }}><Trash2 /></Button></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-xs font-bold">Canal<select value={selected.channel} onChange={(event) => patchEntry(selected.id, { channel: event.target.value as EntryPoint["channel"] })} className={field}><option value="bio">Bio</option><option value="story">Story</option><option value="ad">Anúncio</option><option value="qr">QR</option><option value="linkedin">LinkedIn</option><option value="other">Outro</option></select></label>
          ...
          {[["Origem", "utmSource"], ["Mídia", "utmMedium"], ["Campanha", "utmCampaign"], ["Conteúdo", "utmContent"]].map(([title, key]) => <label key={key} className="text-xs font-bold">{title}<input value={String(selected[key as keyof EntryPoint] || "")} onChange={(event) => patchEntry(selected.id, { [key]: event.target.value || undefined })} className={field} /></label>)}
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="flex min-h-11 items-center gap-2 text-xs font-bold"><input type="checkbox" checked={selected.isActive} onChange={(event) => patchEntry(selected.id, { isActive: event.target.checked })} /> Entrada ativa</label>{selected.surfaceMode === "landing" ? <Link className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#dfdee7] bg-white px-4 text-sm font-semibold text-[#27272c] hover:bg-[#fafafd]" href={`/app/projects/${project.id}/site?compose=landing&entry=${selected.id}`}><Sparkles size={15} />Criar landing para esta entrada</Link> : null}</div>{error ? <p role="alert" className="mt-3 text-sm font-semibold text-red-600">{error}</p> : null}</div>
