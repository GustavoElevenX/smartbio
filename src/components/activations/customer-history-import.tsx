...
export function CustomerHistoryImport({ projectId }: { projectId: string }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[.82fr_1.18fr]">
    <section className="rounded-[24px] border border-[#e4e2ed] bg-white p-6 shadow-[0_16px_50px_rgba(51,45,84,.06)]">
      <span className="grid size-12 place-items-center rounded-2xl bg-[#eeeaff] text-[#604bd8]"><Database size={22} /></span>
      ...
      <label className="mt-6 flex min-h-14 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-[#aaa1df] bg-[#faf9ff] px-4 font-bold text-[#5d49d3]">{busy ? <Loader2 className="animate-spin" size={18} /> : <Upload size={18} />} Enviar CSV<input className="sr-only" type="file" accept=".csv,text/csv" disabled={busy} onChange={(event) => void upload(event.target.files?.[0])} /></label>
      <div className="mt-5 space-y-2">{sources.map((source) => <button key={source.id} type="button" onClick={() => { setSelectedId(source.id); setMapping({ phoneColumn: "", emailColumn: "", externalIdColumn: "" }); }} className={`flex min-h-14 w-full items-center gap-3 rounded-xl border px-4 text-left ${selectedId === source.id ? "border-[#7462e4] bg-[#f5f2ff]" : "border-[#e5e3eb]"}`}><FileSpreadsheet size={18} /><span className="min-w-0 flex-1 truncate text-sm font-bold">{source.name}</span>{selectedId === source.id ? <CheckCircle2 size={18} className="text-[#6752da]" /> : null}</button>)}</div>
    </section>
    <section className="rounded-[24px] border border-[#e4e2ed] bg-white p-6 shadow-[0_16px_50px_rgba(51,45,84,.06)]">
      <p className="text-xs font-extrabold uppercase tracking-[.14em] text-[#6b5bd4]">Mapeamento seguro</p><h2 className="mt-2 text-2xl font-extrabold tracking-[-.035em]">Confirme como identificar cada cliente</h2>
      {preview ? <>{preview.ambiguous ? <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">O CSV tem cabeçalhos ambíguos. Revise o mapeamento antes de importar.</p> : null}<div className="mt-6 grid gap-4 sm:grid-cols-3">{([['phoneColumn','Telefone'],['emailColumn','E-mail'],['externalIdColumn','ID externo']] as const).map(([key,label]) => <label key={key} className="text-sm font-bold">{label}<select value={mapping[key]} onChange={(event) => setMapping((current) => ({ ...current, [key]: event.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-[#dcd9e7] bg-white px-3 font-normal"><option value="">Não mapear</option>{options.map((option) => <option key={option}>{option}</option>)}</select></label>)}</div><div className="mt-6 overflow-x-auto rounded-xl border border-[#e6e4eb]"><table className="min-w-full text-left text-xs"><thead className="bg-[#f5f4f8]"><tr>{options.map((header) => <th key={header} className="px-3 py-3 font-bold">{header}</th>)}</tr></thead><tbody>{preview.rows.slice(0, 5).map((row, index) => <tr key={index} className="border-t border-[#eceaf0]">{row.map((cell, cellIndex) => <td key={cellIndex} className="max-w-48 truncate px-3 py-3 text-[#66646f]">{cell || "—"}</td>)}</tr>)}</tbody></table></div><button type="button" disabled={busy || (!mapping.phoneColumn && !mapping.emailColumn)} onClick={() => void importRows()} className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#5b45de] px-5 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{busy ? <Loader2 className="animate-spin" size={18} /> : <Database size={18} />}Confirmar importação</button></> : <div className="mt-8 rounded-2xl bg-[#f5f4f8] p-8 text-center text-sm text-[#75727d]">Selecione ou envie um CSV para visualizar e mapear as colunas.</div>}
      {message ? <p aria-live="polite" className="mt-4 rounded-xl bg-[#f0edff] p-3 text-sm font-semibold text-[#5746bc]">{message}</p> : null}
    </section>
  </div>;
}
