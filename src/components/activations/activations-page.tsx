...
export function ActivationsPage({ projectId, activations }: { projectId: string; activations: ConversionActivation[] }) {
  ...
  return <div>
    <header className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-sm font-bold text-[#6655dd]">Ativações</p><h1 className="mt-2 text-4xl font-extrabold tracking-[-.045em]">O momento comercial do seu negócio, em movimento.</h1><p className="mt-3 text-[#6e6e78]">Planeje, ative, converta, meça e aprenda — sem reconstruir sua presença.</p></div><div className="flex flex-wrap gap-3"><Link href={`/app/projects/${projectId}/activations/eligibility`} className="focus-ring inline-flex min-h-12 items-center gap-2 rounded-xl border border-[#d8d4ea] bg-white px-5 font-bold text-[#514a71]"><Database size={18} />Base de clientes</Link><Link href={`/app/projects/${projectId}/activations/new`} className="focus-ring inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#5b45de] px-5 font-bold text-white"><Plus size={18} />Nova ativação</Link></div></header>
