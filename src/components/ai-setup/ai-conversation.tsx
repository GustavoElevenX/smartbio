      ...
      <div className="mx-auto max-w-[780px]">
        ...
        <div className="mt-6 flex flex-col gap-5">
          ...
          <Card className="p-5 sm:p-6">
            <fieldset disabled={analyzed || busy} className="mb-5"><legend className="text-sm font-extrabold">O que você precisa colocar no ar?</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{[["business_site", "Uma presença completa para meu negócio"], ["landing_page", "Uma landing page para uma oferta/campanha"], ["conversion_direct", "Uma jornada direta para converter"], ["recommend", "Quero que a Sobe recomende"]].map(([value, text]) => <label key={value} className={`flex min-h-14 cursor-pointer items-center gap-3 rounded-xl border p-3 text-xs font-bold ${form.requestedSurface === value ? "border-[#6d5ef5] bg-[#f1efff]" : "border-[#e1dfe7]"}`}><input type="radio" name="requested-surface" value={value} checked={form.requestedSurface === value} onChange={() => onFormChange({ ...form, requestedSurface: value as InitialSetupForm["requestedSurface"] })} />{text}</label>)}</div></fieldset>
