...
export function AdaptiveQuestion({ question, busy, onAnswer }: { question: SetupQuestion; busy?: boolean; onAnswer: (value: string) => Promise<void> }) {
  ...
  useEffect(() => {
    setValue(question.suggestedAnswer || "");
    setEditingSuggestion(!question.suggestedAnswer);
  }, [question.id, question.suggestedAnswer]);
  async function submit(nextValue = value) { if (nextValue.trim()) await onAnswer(nextValue.trim()); }
  const showSuggestion = Boolean(question.suggestedAnswer && !editingSuggestion);
  return (
