...
export function GenerationStatus({ status }: { status: "idle" | "generating" | "ready" }) {
  ...
  return <div role="status" className="rounded-[18px] border border-[#dedaf4] bg-[#f6f3ff] p-4 text-sm text-[#5449bd]">
    ...
    <p className="mt-2 text-xs leading-5 text-[#74717f]">{status === "generating" ? "A Virou mantém informações não confirmadas como pendências editáveis." : "Revise a experiência na jornada antes de publicar."}</p>
  </div>;
}
