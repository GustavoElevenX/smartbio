...
export default function SetupPreview({ session, businessName, description, brandIdentity, logoPreviewUrl }: { session?: AISetupSession | null; businessName: string; description: string; brandIdentity?: BrandIdentity; logoPreviewUrl?: string }) {
  ...
  const proposedPaths = session?.commercialArchitecture?.intents.filter((intent) => intent.visibleOnEntry).slice(0, 4).map((intent) => intent.label) || [];
  return (
    <aside className="h-full border-l border-[#e7e5ed] bg-[#f7fbff] p-5 xl:p-6">
      <div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 text-xs font-bold text-[#0054fc] shadow-sm"><Eye size={14} /> O que a Sobe entendeu</span><Smartphone size={17} className="text-[#8b8995]" /></div>
      {project ? <div className="mx-auto mt-5 h-[390px] max-w-[230px] overflow-hidden rounded-[30px] border-[5px] border-[#25242a] bg-white shadow-[0_24px_55px_rgba(35,29,72,.16)]"><ExperienceCanvas project={project} preview /></div> : <div className="mx-auto mt-6 max-w-[240px] rounded-[28px] border-[5px] border-[#25242a] bg-white p-2 shadow-[0_24px_55px_rgba(35,29,72,.14)]"><div className="min-h-[350px] rounded-[20px] p-5 transition-colors duration-300" style={{ background: palette?.background || "linear-gradient(155deg,#f7fbff 0%,#fff 52%,#f7fbff 100%)", color: palette?.foreground || "#07172f" }}>{logoPreviewUrl ? <div className="flex h-12 max-w-28 items-center"><img src={logoPreviewUrl} alt="Logo na prévia" className="max-h-12 max-w-full object-contain" /></div> : <span className="grid size-10 place-items-center rounded-xl text-xs font-black" style={{ backgroundColor: palette?.primary || "#0054fc", color: palette?.primaryForeground || "#fff" }}>{(businessName || "SB").slice(0, 2).toUpperCase()}</span>}<h3 className="mt-12 text-2xl font-extrabold leading-[1.08] tracking-[-.04em]">{businessName ? "Escolha seu próximo passo" : "A jornada nasce da conversa."}</h3><p className="mt-3 text-xs leading-5" style={{ color: palette?.mutedForeground || "#526171" }}>{description ? `${description.slice(0, 105)}${description.length > 105 ? "…" : ""}` : "Descreva o negócio para descobrir os caminhos comerciais e as perguntas certas."}</p><div className="mt-5 grid gap-2">{(proposedPaths.length ? proposedPaths : ["Caminhos do seu negócio", "Destino certo", "Só os dados necessários"]).map((item) => <div key={item} className="rounded-xl border p-3 text-xs font-bold" style={{ backgroundColor: palette?.surface || "#fff", borderColor: palette?.border || "#eaf3ff", color: palette?.foreground || "#07172f" }}>{item}</div>)}</div></div></div>}
