...
import { BarChart3, BriefcaseBusiness, ChevronDown, CreditCard, Database, FolderKanban, HelpCircle, Images, LayoutDashboard, LogOut, Menu, Palette, PencilRuler, Settings, Users, X } from "lucide-react";
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const items = [
    ...
    { href: "/app/projects", label: "Projetos", icon: FolderKanban, exact: true },
    ...(projectId ? [
      { href: `/app/projects/${projectId}`, label: "Visão geral do projeto", icon: LayoutDashboard, exact: true },
      { href: `/app/projects/${projectId}/editor`, label: "Editor", icon: PencilRuler },
      ...
      { href: `/app/projects/${projectId}/leads`, label: "Leads", icon: Users },
      { href: `/app/projects/${projectId}/operations`, label: "Operações", icon: BriefcaseBusiness },
      { href: `/app/projects/${projectId}/analytics`, label: "Analytics", icon: BarChart3 },
      { href: `/app/projects/${projectId}/settings`, label: "Configurações do projeto", icon: Settings },
    ] : []),
  ];
  useEffect(() => { const current = localStore.getUser(); if (current) setUser(current); }, []);
