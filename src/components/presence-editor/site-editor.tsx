...
import type {
  SiteComposerIntent,
  SiteOperation,
  SuggestedSiteStructure,
} from "@/features/site-composer/site-composer.types";
...
type PerformanceState = {
  ...
  suggestions: Array<{ id: string; title: string; explanation: string }>;
  publishedAt?: string;
...
function operationDescription(operation: SiteOperation) {
  if (operation.type === "add_page")
    return `Criar página “${operation.page.name}”`;
  if (operation.type === "add_section")
    return `Adicionar ${presenceSectionRegistry[operation.section.sectionType].label}`;
  if (operation.type === "remove_section") return "Remover seção redundante";
  if (operation.type === "move_section")
    return `Mover seção para a posição ${operation.to + 1}`;
  if (operation.type === "update_section")
    return "Atualizar conteúdo, visual e fontes";
  if (operation.type === "rename_page")
    return `Renomear página para “${operation.name}”`;
  return "Conectar objetivo de conversão";
}

function operationSymbol(operation: SiteOperation) {
  if (operation.type === "add_page" || operation.type === "add_section")
    return "+";
...
const sectionLibraryGroups: Array<{
  label: string;
  types: PresenceSectionType[];
}> = [
  { label: "Apresentar", types: ["hero", "about", "rich_text", "video"] },
  {
    label: "Mostrar oferta",
    types: ["products", "services", "pricing", "gallery", "portfolio"],
  },
  {
  ...
  {
    label: "Converter",
    types: ["conversion_cta", "contact", "locations", "divider"],
  },
];
  ...
  return (
    <div className="mt-2 flex flex-col gap-3">
      {sectionLibraryGroups.map((group) => (
        <div key={group.label}>
          <p className="mb-1 text-[10px] font-black uppercase tracking-[.1em] text-[#87909c]">
            {group.label}
          </p>
          <div className="grid grid-cols-2 gap-1">
            {group.types.map((type) => (
              <button
                ...
                key={type}
                onClick={() => onAdd(type)}
                className="min-h-10 rounded-lg p-2 text-left text-[11px] font-bold hover:bg-[#eef6ff]"
              >
                {presenceSectionRegistry[type].label}
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <>
      <button
        ...
        onClick={onClose}
        className="fixed inset-0 z-40 bg-[#101827]/45 xl:hidden"
      />
      <section
        role="dialog"
        aria-modal="true"
        aria-label={title}
        className="fixed inset-x-0 bottom-0 z-50 max-h-[82vh] overflow-y-auto rounded-t-[28px] bg-white p-4 shadow-[0_-20px_70px_rgba(15,23,42,.28)] xl:hidden"
      >
...
export function SiteEditor({ projectId }: { projectId: string }) {
  ...
  const [structureProposal, setStructureProposal] =
    useState<StructureProposal>();
  const [aiInstruction, setAiInstruction] = useState("");
  const [aiIntent, setAiIntent] =
    useState<SiteComposerIntent>("suggest_structure");
  ...
  const [performanceExplanations, setPerformanceExplanations] = useState<
    Record<string, PerformanceExplanation>
  >({});
  const [performanceLoadingId, setPerformanceLoadingId] = useState<string>();
  const [approvedSuggestionIds, setApprovedSuggestionIds] = useState<
    Set<string>
  >(new Set());
  ...
  useEffect(() => {
    ...
    void fetch(`/api/projects/${performanceProjectId}/optimization`)
      .then((response) => response.json())
      .then((payload: { data?: PerformanceState }) =>
  ...
  const qualityWarnings = useMemo(
    () => (activePage ? inspectPageQuality(activePage) : []),
    [activePage],
  );
  ...
  async function requestStructure(
    intent: SiteComposerIntent = aiIntent,
    ...
    try {
      ...
      const instruction =
        ...
        aiInstruction.trim() ||
        copilotActions.find((action) => action.intent === intent)?.label ||
        "Organize o site para conduzir ao objetivo comercial principal.";
      const response = await fetch(
        `/api/ai/projects/${project.id}/site/suggest-structure`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            ...
            intent,
            target,
            pageId: target === "page" ? activePage?.id : undefined,
          }),
        },
      );
      const payload = (await response.json()) as {
      ...
      if (!response.ok || !payload.data)
        throw new Error(
          payload.error?.message || "Não foi possível sugerir a estrutura.",
        );
      setStructureProposal(payload.data);
      setSelectedOperationIds(
        new Set(payload.data.operations.map((operation) => operation.id)),
      );
      ...
      setAiError(
        error instanceof Error
          ? error.message
          : "Não foi possível sugerir a estrutura.",
      );
    } finally {
      setAiLoading(false);
    }
  ...
  async function explainPerformanceSuggestion(suggestionId: string) {
    ...
    try {
      const response = await fetch(
        `/api/projects/${project.id}/optimization/explain`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
      ...
      const payload = (await response.json()) as {
        data?: PerformanceExplanation;
        error?: string;
      };
      if (!response.ok || !payload.data)
        throw new Error(
          payload.error || "Não foi possível explicar esta evidência.",
        );
      setPerformanceExplanations((current) => ({
        ...current,
        [suggestionId]: payload.data!,
      }));
    } catch (error) {
      setAiError(
        error instanceof Error
          ? error.message
          : "Não foi possível explicar esta evidência.",
      );
    } finally {
      setPerformanceLoadingId(undefined);
    }
  }
  async function approvePerformanceSuggestion(suggestionId: string) {
    if (!project) return;
    setPerformanceLoadingId(suggestionId);
    setAiError("");
    try {
      const response = await fetch(`/api/projects/${project.id}/optimization`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ suggestionId }),
      });
      ...
      if (!response.ok || !payload.data?.approved)
        throw new Error(
          payload.error?.message || "Não foi possível aprovar o teste.",
        );
      setApprovedSuggestionIds((current) => new Set(current).add(suggestionId));
      setMessage(
        "Teste aprovado. A próxima publicação vinculará a nova versão à janela de observação.",
      );
    } catch (error) {
      setAiError(
        error instanceof Error
          ? error.message
          : "Não foi possível aprovar o teste.",
      );
    } finally {
      setPerformanceLoadingId(undefined);
    }
  ...
  async function applyStructureProposal() {
    ...
    try {
      ...
      if (!selectedOperations.length)
        throw new Error("Selecione pelo menos uma operação para aplicar.");
      const response = await fetch(
        `/api/projects/${project.id}/site/apply-proposal`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            proposalId: structureProposal.proposalId,
            selectedOperations,
            expectedVersion: structureProposal.expectedVersion,
          }),
        },
      );
      const payload = (await response.json()) as {
        data?: { pages?: PresencePage[] };
        error?: { message?: string };
      };
      if (!response.ok)
        throw new Error(
          payload.error?.message || "Não foi possível aplicar a proposta.",
        );
      if (payload.data?.pages?.length) {
        const changedPages = payload.data.pages;
        setPages((current) => {
          const changedById = new Map(
            changedPages.map((page) => [page.id, page]),
          );
          const next = current.map((page) => changedById.get(page.id) || page);
          for (const page of changedPages)
            if (!next.some((candidate) => candidate.id === page.id))
              next.push(page);
          setProject((currentProject) =>
            currentProject
              ? { ...currentProject, presence: { pages: next } }
              : currentProject,
          );
          return next;
        });
        ...
        if (refreshed) {
          ...
          setPages(refreshed.presence?.pages || []);
        }
      }
      setStructureProposal(undefined);
      setMessage(
        "Proposta aplicada somente ao rascunho. Revise antes de publicar.",
      );
    } catch (error) {
      setAiError(
        error instanceof Error
      ...
      <div className="grid min-h-[620px] place-items-center rounded-[28px] border border-[#e2e0e8] bg-white p-8 text-center shadow-sm">
        <div className="max-w-md">
          ...
          <p className="mt-3 text-sm leading-6 text-[#706d78]">
            Monte uma página conectada às ações que seus clientes precisam
            realizar.
          </p>
  ...
  return (
    <div className="-m-4 min-h-[calc(100vh-73px)] bg-[#f1f5f9] sm:-m-6 lg:-m-8">
      <div className="flex h-16 items-center gap-2 border-b border-[#dfe6ee] bg-white px-3 sm:gap-3 sm:px-4">
        ...
        <div className="hidden min-w-0 md:block">
          <div className="flex items-center gap-2">
            <h1 className="truncate text-sm font-black">Minha página</h1>
            <span
      ...
      {mode === "simple" && pages.length > 1 ? (
        <div className="flex items-end gap-3 border-b border-[#dfe6ee] bg-white px-4 py-3">
          <label className="min-w-0 flex-1 text-xs font-extrabold text-[#536178]">
            Página atual
            <select
              className="mt-1 min-h-11 w-full border border-[#d7e0e9] bg-white px-3 text-sm"
              value={activePage?.id || ""}
              onChange={(event) => {
          ...
          <details className="relative">
            <summary className="flex min-h-11 cursor-pointer list-none items-center px-3 text-xs font-extrabold text-[#0054fc]">
              Mais opções
            </summary>
            <div className="absolute right-0 top-12 z-40 w-56 border border-[#dfe6ee] bg-white p-2 shadow-xl">
              <button
                ...
                className="min-h-11 w-full px-3 text-left text-sm font-bold hover:bg-[#f1f5f9]"
              >
                Adicionar página
              </button>
              <button
      ...
      {mode === "advanced" ? (
        <div className="flex items-center gap-2 border-b border-[#dfe6ee] bg-white p-3 xl:hidden">
          <label className="min-w-0 flex-1 text-xs font-black text-[#5f6673]">
            Página ativa
            <select
              className="mt-1 min-h-11 w-full rounded-xl border border-[#d7e0e9] bg-white px-3 text-sm"
              value={activePage?.id || ""}
              onChange={(event) => {
                setSelectedPageId(event.target.value);
                setSelectedSectionId(undefined);
      ...
      {mode === "advanced" && !mobilePanel ? (
        <div className="grid grid-cols-3 gap-1 border-b border-[#d7e0e9] bg-white p-2 xl:hidden">
          <button
            type="button"
            onClick={() => setMobilePanel("pages")}
            className="min-h-11 rounded-xl text-xs font-black text-[#53606d] hover:bg-[#eef6ff]"
          >
            Páginas
          </button>
          <button
          ...
          <button
            ...
            className="min-h-11 rounded-xl bg-[#0054fc] text-xs font-black text-white"
          >
            Propriedades
          </button>
        </div>
      ) : null}
      {mode === "advanced" && mobilePanel === "pages" ? (
        <MobileDrawer title="Páginas" onClose={() => setMobilePanel(undefined)}>
          <div className="space-y-1">
            {pages.map((page) => (
              <button
                ...
                className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-bold ${page.id === activePage?.id ? "bg-[#e8f4ff] text-[#0054fc]" : "hover:bg-[#f1f5f9]"}`}
              >
                <span>
                  {page.isHome ? "⌂" : page.type === "landing" ? "LP" : "P"}
                </span>
      ...
      {mode === "advanced" && mobilePanel === "sections" ? (
        <MobileDrawer
          title="Conteúdo desta página"
        ...
        >
            ...
            {activePage?.sections
              ...
              .map((section) => (
                <button
                  ...
                  className={`flex min-h-11 w-full items-center gap-2 rounded-xl px-3 text-left text-xs font-bold ${section.id === activeSection?.id ? "bg-[#e8f4ff] text-[#0054fc]" : "hover:bg-[#f1f5f9]"}`}
                >
          ...
          <details className="mt-4">
            <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-xl border border-dashed border-[#b9cce1] px-3 text-xs font-black text-[#0054fc]">
              <Plus size={14} />
              Adicionar conteúdo
            </summary>
            <SectionLibrary
      ...
      <div
        data-testid={`site-editor-${mode}`}
        className={`grid min-h-[calc(100vh-137px)] grid-cols-1 ${mode === "advanced" ? "xl:grid-cols-[240px_minmax(380px,1fr)_320px]" : "lg:grid-cols-[minmax(480px,1fr)_340px]"}`}
      >
        <aside
          className={
            mode === "advanced"
              ? "hidden border-r border-[#dfe6ee] bg-white xl:block"
              : "hidden"
          }
        >
          ...
          <div className="mx-3 mt-4 border-t border-[#e3e9ef] pt-4">
            <div className="flex items-center justify-between px-1">
              <strong className="text-xs uppercase tracking-[.12em] text-[#697180]">
                ...
                .map((section) => (
                  ...
                  >
                    <button
                      ...
                      className={`flex min-h-10 w-full items-center gap-2 rounded-xl px-2 text-left text-xs font-bold ${section.id === activeSection?.id ? "bg-[#e8f4ff] text-[#0054fc]" : "text-[#505866] hover:bg-[#f1f5f9]"}`}
                    >
                      <GripVertical
                        size={14}
                        className="cursor-grab text-[#a6afbb]"
                      />
                      <span className="min-w-0 flex-1 truncate">
                        {section.title ||
                          presenceSectionRegistry[section.type].label}
            ...
            <details className="mt-3">
              <summary className="flex min-h-10 cursor-pointer list-none items-center gap-2 rounded-xl border border-dashed border-[#b9cce1] px-3 text-xs font-black text-[#0054fc]">
                <Plus size={14} />
                Adicionar conteúdo
              </summary>
        ...
        {mode === "simple" && activeSection ? (
          <button
            type="button"
            data-testid="simple-editor-backdrop"
            aria-label="Fechar edição"
            onClick={() => setSelectedSectionId(undefined)}
            className="fixed inset-0 z-[35] bg-[#101827]/45 lg:hidden"
          />
        ) : null}
        {mode === "simple" ? (
          <aside
            ...
            {activeSection && activePage ? (
              <>
                ...
                <SimpleSectionInspector
                  ...
                  onMove={(direction) =>
                    moveSection(activeSection.id, direction)
                  }
              ...
              <div className="flex min-h-64 flex-col justify-center">
                <strong className="text-xl font-black tracking-[-.025em]">
                  Clique em uma parte da página
                </strong>
                <p className="mt-3 text-sm leading-6 text-[#65717d]">
                  Você poderá editar título, texto, imagem e botões sem lidar
                  com configurações técnicas.
                </p>
                <Button
                  className="mt-5 self-start"
                  variant="secondary"
                  onClick={() => void requestAI("page")}
                >
                  <Sparkles size={16} /> Melhorar esta página
                </Button>
              </div>
            )}
          </aside>
        ) : null}
        {mode === "advanced" && mobilePanel === "properties" ? (
          <button
            type="button"
            aria-label="Fechar painel"
            onClick={() => setMobilePanel(undefined)}
            className="fixed inset-0 z-[35] bg-[#101827]/45 xl:hidden"
          />
        ) : null}
        <aside
          role={mobilePanel === "properties" ? "dialog" : undefined}
          aria-modal={mobilePanel === "properties" ? true : undefined}
          aria-label={mobilePanel === "properties" ? "Propriedades" : undefined}
          className={
            mode === "advanced"
              ? `border-l border-[#dfe6ee] bg-white ${mobilePanel === "properties" ? "fixed inset-x-0 bottom-0 z-40 max-h-[82vh] overflow-hidden rounded-t-[28px] shadow-[0_-20px_70px_rgba(15,23,42,.28)]" : "hidden"} xl:static xl:block xl:max-h-none xl:rounded-none xl:shadow-none`
              : "hidden"
          }
        >
          ...
          <Tabs
            ...
            className="max-h-[calc(100vh-185px)] overflow-y-auto"
          >
            <TabsList className="sticky top-0 z-10 grid h-auto w-full grid-cols-4 rounded-none border-b border-[#e3e9ef] bg-white p-2">
              <TabsTrigger value="content" className="px-1 text-[11px]">
                Conteúdo
              </TabsTrigger>
              <TabsTrigger value="visual" className="px-1 text-[11px]">
                Aparência
              </TabsTrigger>
              <TabsTrigger value="conversion" className="px-1 text-[11px]">
                Ação
              </TabsTrigger>
              <TabsTrigger value="advanced" className="px-1 text-[11px]">
                Avançado
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="visual" className="m-0 p-4">
              <div className="rounded-2xl border border-[#dfe7ef] bg-[#f8fbfe] p-4">
                <strong className="text-sm">Visual controlado</strong>
                <p className="mt-2 text-xs leading-5 text-[#66717e]">
                  Escolha composição, largura, ritmo, fundo e tratamento de
                  mídia. CSS arbitrário não é aceito.
                </p>
              </div>
              {activeSection && activePage ? (
                <div className="mt-4">
            ...
            <TabsContent value="conversion" className="m-0 p-4">
              <div className="rounded-2xl border border-[#dbe8f5] bg-[#eff7ff] p-4">
                <strong className="text-sm">Objetivo da página</strong>
                <p className="mt-2 text-xs leading-5 text-[#5d6c7b]">
                  {activePage?.defaultConversionGoalId
                    ? "Esta página está conectada a um objetivo mensurável."
                    : "Conecte esta página a um objetivo antes de publicar."}
                </p>
              </div>
              {activePage ? (
                <label className="mt-4 block text-xs font-bold text-[#555f6b]">
                  Objetivo principal
                  <select
                    value={activePage.defaultConversionGoalId || ""}
                    onChange={(event) =>
                      updatePage({
                        ...activePage,
                        defaultConversionGoalId:
                          event.target.value || undefined,
                      })
                    }
                    className="mt-2 min-h-11 w-full rounded-xl border border-[#d9e1e9] bg-white px-3 text-sm"
                  >
            ...
            <TabsContent value="advanced" className="m-0 p-4">
              <div className="rounded-2xl border border-[#dfe7ef] p-4">
                <strong className="text-sm">Fontes conectadas</strong>
                <dl className="mt-3 grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <dt className="text-[#75808c]">Produtos</dt>
                    <dd className="mt-1 font-black">
                      {project.commercialConfig?.catalogItems?.length || 0}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-[#75808c]">Serviços</dt>
                    <dd className="mt-1 font-black">
                      {project.commercialConfig?.serviceOfferings?.length || 0}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-[#75808c]">Unidades</dt>
                    <dd className="mt-1 font-black">
                      {project.commercialConfig?.locations?.length || 0}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-[#75808c]">Objetivos</dt>
                    <dd className="mt-1 font-black">
                      {project.conversionGoals?.length || 0}
                    </dd>
                  </div>
                </dl>
              </div>
              <div className="mt-4 rounded-2xl bg-[linear-gradient(135deg,#edf8ff,#e7f1ff)] p-4">
                <span className="flex items-center gap-2 text-xs font-black text-[#0f64c8]">
                  <Sparkles />
                  Sobe IA
                </span>
                <h3 className="mt-3 font-black">O que você quer fazer?</h3>
                <p className="mt-2 text-xs leading-5 text-[#526476]">
                  Explique o foco, a oferta, o público ou a página. Planner e IA
                  usam os dados reais do negócio; nada é publicado
                  automaticamente.
                </p>
                <textarea
                  aria-label="Instrução para a Sobe IA"
                  value={aiInstruction}
                  onChange={(event) => setAiInstruction(event.target.value)}
                  placeholder="Ex.: Quero uma landing para revendedores e menos destaque para o FAQ."
                  className="mt-4 min-h-28 w-full resize-y rounded-xl border border-[#b9d2eb] bg-white p-3 text-sm leading-6 outline-none focus:border-[#0054fc] focus:ring-4 focus:ring-[#0054fc]/10"
                />
                <div className="mt-3 flex flex-wrap gap-2">
                  {copilotActions.map((action) => (
                    <button
                      key={action.intent}
                      type="button"
                      onClick={() => setAiIntent(action.intent)}
                      className={`min-h-9 rounded-full border px-3 text-[11px] font-black ${aiIntent === action.intent ? "border-[#0054fc] bg-[#0054fc] text-white" : "border-[#b9d2eb] bg-white text-[#36536f]"}`}
                    >
                      {action.label}
                    </button>
                  ))}
                ...
                <Button
                  className="mt-4 w-full bg-[#0054fc] hover:bg-[#0d54a9]"
                  size="sm"
                  disabled={aiLoading}
                  onClick={() => void requestStructure(aiIntent)}
                >
                  {aiLoading ? (
                    <LoaderCircle className="animate-spin" />
                  ) : (
            ...
            <TabsContent
              value="advanced"
              className="m-0 border-t border-[#e3e9ef] p-4"
            >
              <SurfaceViewMarker surface="optimization" projectId={project.id} />
              <div className="rounded-2xl bg-[#f7fbff] p-4">
                <strong className="text-sm">Quality Assistant</strong>
                <p className="mt-2 text-xs leading-5 text-[#666174]">
                  Diagnóstico da página ativa. Ele orienta; não altera nem
              ...
              <div className="mt-4 flex flex-col gap-2">
                {qualityWarnings.length ? (
                  qualityWarnings.map((warning) => (
                    <div
                      key={warning.code}
                      className="rounded-xl border border-[#eaf3ff] bg-white p-3"
                    >
                      <strong className="text-xs">{warning.message}</strong>
                      <p className="mt-1 text-[11px] text-[#746f7d]">
                        Selecione a seção relacionada ou peça uma proposta à
                        Sobe IA.
                      </p>
                    </div>
                  ))
                  ...
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs font-bold text-emerald-800">
                    Nenhum problema estrutural detectado nesta página.
                  </div>
                )}
              </div>
            </TabsContent>
            ...
            >
              <div className="rounded-2xl bg-[#eef6ff] p-4">
                <strong className="text-sm">Performance Copilot</strong>
                <p className="mt-2 text-xs leading-5 text-[#566a7e]">
                  Sugestões só aparecem após 30 dias, 30 sessões e 15 sessões da
                  meta principal quando aplicável.
                </p>
              </div>
              {!performance?.publishedAt ? (
                <p className="mt-4 rounded-xl border border-[#dfe7ef] p-4 text-xs leading-5 text-[#697684]">
                  Publique o site para iniciar a janela de aprendizado.
                </p>
              ) : performance.evidence ? (
                <div className="mt-4 flex flex-col gap-3">
                  <p
                    className={`rounded-xl p-3 text-xs font-bold ${performance.evidence.eligible ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}
                  >
                    {performance.evidence.message}
                  </p>
                  <EvidenceBar
                    label="Dias"
                    progress={performance.evidence.daysProgress}
                    value={`${performance.evidence.completeDays}/30`}
                  />
                  <EvidenceBar
                    label="Sessões"
                    progress={performance.evidence.sessionsProgress}
                    value={`${Math.round(performance.evidence.sessionsProgress * 30)}/30`}
                  />
                  {performance.evidence.goalSessionsProgress != null ? (
                    <EvidenceBar
                      label={performance.primaryGoalName || "Meta principal"}
                      progress={performance.evidence.goalSessionsProgress}
                      value={`${Math.round(performance.evidence.goalSessionsProgress * 15)}/15`}
                    />
                  ) : null}
                  {performance.suggestions.map((suggestion) => {
                    const explanation = performanceExplanations[suggestion.id];
                    const proposalInstruction = `Proponha uma alteração para “${suggestion.title}”. Evidência observada: ${suggestion.explanation}.${explanation ? ` Leitura: ${explanation.explanation} Próxima ação: ${explanation.recommendedAction}` : ""}`;
                    return (
                      <article
                        key={suggestion.id}
                        className="rounded-xl border border-[#dfe7ef] p-3"
                      >
                        <strong className="text-xs">{suggestion.title}</strong>
                        <p className="mt-2 text-[11px] leading-5 text-[#697684]">
                          {suggestion.explanation}
                        </p>
                        {explanation ? (
                          <div className="mt-3 rounded-xl bg-[#f7fbff] p-3">
                            <span className="text-[10px] font-black uppercase tracking-[.08em] text-[#0054fc]">
                              {explanation.usedAI
                                ? "Leitura da Sobe IA"
                                : "Leitura determinística"}
                            </span>
                            <p className="mt-2 text-[11px] leading-5 text-[#5f5970]">
                              {explanation.explanation}
                            </p>
                            <p className="mt-2 text-[11px] font-bold text-[#4e4861]">
                              Próxima ação: {explanation.recommendedAction}
                            </p>
                          </div>
                        ) : null}
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Button
                            size="sm"
                            variant="ghost"
                            disabled={performanceLoadingId === suggestion.id}
                            onClick={() =>
                              void explainPerformanceSuggestion(suggestion.id)
                            }
                          >
                            {performanceLoadingId === suggestion.id ? (
                              <LoaderCircle className="animate-spin" />
                            ) : (
                              <Sparkles />
                            )}
                          ...
                          <Button
                            ...
                            disabled={aiLoading}
                            onClick={() => {
                              setAiInstruction(proposalInstruction);
                              setAiIntent("reorganize");
                          ...
                          <Button
                            ...
                            disabled={
                              performanceLoadingId === suggestion.id ||
                              approvedSuggestionIds.has(suggestion.id)
                            }
                            onClick={() =>
                              void approvePerformanceSuggestion(suggestion.id)
                            }
                          >
                            {approvedSuggestionIds.has(suggestion.id)
                              ? "Teste aprovado"
                              : "Aprovar teste manual"}
                          </Button>
                        </div>
                      </article>
                    );
                ...
                <p className="mt-4 text-xs text-[#697684]">
      ...
      {structureProposal ? (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="structure-proposal-title"
          className="fixed inset-0 z-50 grid place-items-center bg-[#101827]/55 p-4"
        >
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-[0_28px_90px_rgba(15,23,42,.34)] md:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="inline-flex items-center gap-2 text-xs font-black text-[#0054fc]">
                  <Sparkles />
                  Sobe IA ·{" "}
                  {structureProposal.usedAI
                    ? "planner + IA contextual"
                    : "planner determinístico"}
                </span>
                <h2
                  id="structure-proposal-title"
                  className="mt-2 text-2xl font-black"
                >
                  Revise a proposta
                </h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#66717e]">
                  {structureProposal.suggestion.reasoning}
                </p>
              </div>
              <Button
                size="icon"
                ...
                onClick={() => setStructureProposal(undefined)}
              >
                <X />
              </Button>
            </div>
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {structureProposal.suggestion.pages.map((page) => (
                <article
                  key={`${page.type}-${page.pathSuggestion}`}
                  className="rounded-2xl border border-[#dfe7ef] p-4"
                >
                  ...
                  <p className="mt-2 text-xs leading-5 text-[#687582]">
                    {page.purpose}
                  </p>
                  <ol className="mt-3 flex flex-col gap-1 text-xs">
                    {page.sections.map((section, index) => (
                      ...
                      >
                        <span className="text-[#0054fc]">{index + 1}.</span>
                        <span>
                          {presenceSectionRegistry[section.sectionType].label} —{" "}
                          {section.purpose}
                        </span>
                      </li>
                    ))}
            ...
            {structureProposal.suggestion.warnings.length ? (
              <div className="mt-4 flex flex-col gap-1 rounded-2xl bg-amber-50 p-4 text-sm text-amber-900">
                {structureProposal.suggestion.warnings.map((warning) => (
                  <p key={warning}>{warning}</p>
                ))}
              </div>
            ) : null}
            <div className="mt-5 rounded-2xl border border-[#dfe7ef] bg-[#f8fafc] p-4">
              <div className="flex items-center justify-between gap-3">
                <strong className="text-sm">Diff semântico</strong>
                <button
                  type="button"
                    ...
                    setSelectedOperationIds(
                      new Set(
                        selectedOperationIds.size ===
                          structureProposal.operations.length
                          ? []
                          : structureProposal.operations.map(
              ...
              <div
                className={`mt-3 flex flex-col gap-2 ${customizingProposal ? "" : "max-h-52 overflow-hidden"}`}
              >
                {structureProposal.operations.map((operation) => (
                  <label
                    key={operation.id}
                    className="flex min-h-11 items-center gap-3 rounded-xl border border-[#dde6ee] bg-white px-3 py-2 text-xs font-bold"
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <span
                      className={`grid size-6 shrink-0 place-items-center rounded-lg ${operation.type === "remove_section" ? "bg-red-50 text-red-700" : operation.type === "move_section" ? "bg-amber-50 text-amber-800" : "bg-blue-50 text-[#0054fc]"}`}
                    >
              ...
              {structureProposal.operations.length > 4 ? (
                <button
                  type="button"
                  onClick={() => setCustomizingProposal((value) => !value)}
                  className="mt-3 text-xs font-black text-[#0054fc]"
                >
                  {customizingProposal
                    ? "Mostrar menos"
                    : "Ver e personalizar todas as operações"}
                </button>
              ) : null}
            </div>
            <p className="mt-4 text-xs font-bold text-[#65717d]">
              {selectedOperationIds.size} de{" "}
              {structureProposal.operations.length} operações selecionadas. A
              aplicação altera apenas o rascunho e nunca publica o site.
            </p>
            <div className="mt-6 flex flex-wrap justify-end gap-2">
              ...
              <Button
                variant="secondary"
                onClick={() => setCustomizingProposal(true)}
              >
                Personalizar antes
              </Button>
              <Button
                disabled={aiLoading || !selectedOperationIds.size}
                onClick={() => void applyStructureProposal()}
              >
                {aiLoading ? <LoaderCircle className="animate-spin" /> : null}
                Aplicar ao rascunho
              </Button>
            </div>
          </div>
...
function EvidenceBar({
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between gap-3 text-[11px] font-bold text-[#5f6d7b]">
        <span>{label}</span>
        <span>{value}</span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-[#dfe8f1]">
        <div
    ...
    <div className="space-y-4">
      ...
      <label className={label}>
        ...
        <textarea
          ...
          maxLength={400}
          onChange={(event) =>
            onChange({ ...page, purpose: event.target.value || undefined })
          }
          placeholder="Explique qual decisão esta página deve apoiar."
        />
      </label>
      <label className={label}>
        Caminho
