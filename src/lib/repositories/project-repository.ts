...
function normalizeIds(project: Project): Project {
  ...
  if (commercialConfig) {
    ...
    for (const collection of [commercialConfig.schedulableServices, commercialConfig.resources, commercialConfig.availabilityRules, commercialConfig.availabilityExceptions, commercialConfig.catalogCategories, commercialConfig.catalogItems, commercialConfig.reservableUnits, commercialConfig.reservationBlocks, commercialConfig.locations, commercialConfig.routingRules]) {
...
async function saveNormalized(project: Project) {
  ...
  if (config?.schedulableServices?.length) {
    const { error } = await supabase.from("schedulable_services").upsert(config.schedulableServices.map((service) => ({
      id: service.id, project_id: normalized.id, name: service.name, duration_minutes: service.durationMinutes,
  ...
  if (config?.routingDestinations?.length) {
    const { error } = await supabase.from("routing_destinations").upsert(config.routingDestinations.map((destination) => ({
      ...
      channel: destination.type === "whatsapp" ? "whatsapp" : destination.type === "url" ? "url" : "internal",
      value: destination.value || destination.key, is_active: true,
    })));
    if (error) throw new Error(error.message);
  }
...
export const projectRepository = {
  async getProjects(): Promise<Project[]> {
    ...
    const supabase = createClient();
    if (!supabase) return localStore.getProjects();
    const { data: userData } = await supabase.auth.getUser();
    if (!userData.user) return localStore.getProjects();
  ...
  async getProject(value: string): Promise<Project | undefined> {
    if (!isSupabaseConfigured()) return localStore.getProject(value);
    const supabase = createClient();
    if (!supabase) return localStore.getProject(value);
  ...
  async saveProject(project: Project): Promise<Project> {
    ...
    if (remote.id !== local.id) localStore.deleteProject(local.id);
