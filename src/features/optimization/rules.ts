...
export function goalDropoffRule(input: { projectId: string; goalId: string; goalName: string; totalSessions: number; goalSessions: number; actionSessions: number }): OptimizationSuggestion | null {
  if (!hasEnoughEvidence(input.totalSessions, input.goalSessions)) return null;
