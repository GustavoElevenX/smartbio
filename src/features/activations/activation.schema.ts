...
const activationBaseSchema = z.object({
  ...
  status: z.enum(["draft","scheduled","active","paused","ended","archived"]).default("draft"), conversionGoalId: z.string().trim().min(1).max(150).optional(), defaultDestinationId: z.string().trim().min(1).max(150).optional(), destinationMode: z.enum(["routing","fixed_destination","goal_default","native"]).default("fixed_destination"),
  ...
  entryPointIds: z.array(z.string().uuid()).default([]), locationIds: z.array(z.string().uuid()).default([]), publishedSnapshot: z.record(z.string(), z.unknown()).optional(), publishedAt: z.iso.datetime().optional(), version: z.number().int().positive().default(1), publishedVersion: z.number().int().positive().optional(), createdAt: z.iso.datetime().optional(), updatedAt: z.iso.datetime().optional(),
});
