...
export interface ConversionFunnelStage {
  key: "attention" | "intention" | "action" | "opportunity" | "conversion";
  label: string;
  sessions: number;
  rateFromAttention: number;
  rateFromPrevious: number;
}
const actionEvents: AnalyticsEvent["eventName"][] = [
...
const sessionSet = (
  ...
  names: AnalyticsEvent["eventName"][],
) =>
  new Set(
    events
...
export function buildConversionFunnel(
  ...
  const attention = sessionSet(events, [
    "page_view",
    "presence_page_viewed",
    "session_started",
  ]);
  const intention = sessionSet(events, [
  ...
  return sets.map((set, index) => ({
    ...
    label: labels[index],
    sessions: set.size,
    rateFromAttention: attention.size
      ? Math.round((set.size / attention.size) * 1000) / 10
      : 0,
    rateFromPrevious:
      index === 0
        ? 100
        : sets[index - 1].size
          ? Math.round((set.size / sets[index - 1].size) * 1000) / 10
          : 0,
  }));
}
