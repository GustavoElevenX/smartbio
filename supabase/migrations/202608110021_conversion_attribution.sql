alter table public.visitor_sessions
  ...
  add column if not exists destination_id uuid references public.routing_destinations(id) on delete set null;
...
create index if not exists visitor_sessions_project_started_idx on public.visitor_sessions(project_id, started_at desc);
create index if not exists visitor_sessions_goal_started_idx on public.visitor_sessions(project_id, conversion_goal_id, started_at desc);
create index if not exists visitor_sessions_entry_started_idx on public.visitor_sessions(project_id, entry_point_id, started_at desc);
create index if not exists analytics_events_project_created_idx on public.analytics_events(project_id, created_at desc);
create index if not exists analytics_events_project_goal_created_idx on public.analytics_events(project_id, conversion_goal_id, created_at desc);
create index if not exists analytics_events_project_entry_created_idx on public.analytics_events(project_id, entry_point_id, created_at desc);
create index if not exists analytics_events_project_destination_created_idx on public.analytics_events(project_id, destination_id, created_at desc);
